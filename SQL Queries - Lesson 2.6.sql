...
SELECT	last_name
        , COUNT(last_name) AS count_of_l_name
FROM sakila.actor
GROUP BY 1
HAVING count_of_l_name = 1
;
...
SELECT	staff_id
		, COUNT(staff_id) AS '#_of_rentals_processed' 
FROM sakila.rental
GROUP BY 1
;
...
SELECT	release_year
		, COUNT(release_year) AS '#_of_movies'
FROM sakila.film
GROUP BY 1
;
...
SELECT	title
		, length
        , RANK() OVER (ORDER BY length) AS film_ranked     
FROM sakila.film
